...
function tabla() {
    $.post(
            base_url + "MantenedorUsuario/tabla",
            {},
            function(ruta, datos) {            	
                $("#tabla").hide();
...
function cargarEditar(id_contMenu){
	cargando('mostrar');
    $.post(
        base_url + "MantenedorUsuario/cargarEditarUsuario", 
...
function Deshabilitar(id_contMenu){
    cargando('mostrar');
    $.post(
        base_url + "MantenedorUsuario/Deshabilitar", 
            {id_contMenu:id_contMenu}, 
            function(datos) {               
                if(datos.valor==0){
...
function Habilitar(id_contMenu){
    cargando('mostrar');
    $.post(
        base_url + "MantenedorUsuario/Habilitar", 
            {id_contMenu:id_contMenu}, 
            function(datos) {               
                if(datos.valor==0){
                    clickMenu('MantenedorUsuario');
...
$('#btnFiltrar').click(function(){
    ...
    $.post(
        ...
        base_url + 'MantenedorUsuario/filtrar',
...
$(".botonAgregarRegistro").click(function() {
    cargando('mostrar');
    $.post(
        base_url + "MantenedorUsuario/cargarCrearUsuario", 
            {}, 
            function (ruta) {                                 
        $("#content").html(ruta);        
    });           
